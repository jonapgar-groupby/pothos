...
function addTypeSelectionsForField(
  ...
  if (pothosDrizzleSelect) {
    mergeSelection(state, { ...pothosDrizzleSelect });
  }
    ...
    fieldSelectionMap = fieldSelect(
      ...
      (rawQuery, indirectInclude) => {
        ...
        const normalizedIndirectInclude = Array.isArray(indirectInclude)
          ...
          : indirectInclude;
        const fieldState = createStateForSelection(state);
        if (typeof query === 'object' && Object.keys(query).length > 0) {
          mergeSelection(fieldState, { columns: {}, ...query });
        }
        if (
      ...
      (indirectType, indirectField, subPath) => {
        resolveIndirectIncludePaths(
          ...
          (resolvedType, resolvedField, nested) => {
            state = createStateForSelection(undefined, select);
...
export function selectionStateFromInfo(
  ...
  const state = createStateForSelection();
...
function createStateForSelection(parent?: SelectionState, initialSelections?: SelectionMap) {
